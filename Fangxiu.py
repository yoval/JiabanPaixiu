...
layout = [  
    ...
    [sg.Multiline('放休表姓名',size=(40,10), key='FangNames'),sg.Multiline('放休表时段',size=(40,10), key='FangPaiban')],
    [sg.Button('整合排班')],
    [sg.Output(size=(85, 20))],
    ]
window = sg.Window('放休排班表整合 beta 版', layout)
    ...
    try:
        ...
        for df_Fang_row in df_Fang.itertuples():
            ...
            try:
                ...
                NewShiduan = list(set(to_shijiandian(Yuanshiduan)) - set(to_shijiandian(shiduan)))
                NewShiduan = to_shijianduan(NewShiduan)
                df_Yuan.at[name,date] = NewShiduan
            except :
                print('%s 班次修改错误'%name)
        df_Yuan.to_csv('放休之后的班表-%s.csv'%now,encoding="utf_8_sig",index=True)
    except:
        print('输入有误！请检查输入的数据！')
window.close()